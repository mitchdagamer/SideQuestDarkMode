class App {
    constructor() {
        ...
        this.filter_select.addEventListener('change', () =>
            this.searchFilter()
        );
                        ...
                        this.bsaber
                            ...
                            .then(id => {
                                this.getMySongs()
                                    ...
                                    .then(() =>
                                        this.showStatus('Level Downloaded Ok!!')
                                    )
                                    .then(() =>
                                        this.bsaber.getCurrentDeviceSongs(true)
                                    )
                                    .then(() => {
        ...
        M.AutoInit();
        this.mkdir(appData)
            .then(() => this.mkdir(path.join(appData, 'apk-backups')))
            ...
            .then(async () => {
                if (!fs.existsSync(path.join(appData, 'sources.txt'))) {
                    fs.createReadStream(
                        path.join(__dirname, 'sources.txt')
                    ).pipe(
                ...
                this.toggleLoader(true);
                this.spinner.setMessage(
                    'Downloading, This might take some time on first launch please wait...'
                );
    ...
    setupMenu() {
        document
            .querySelector('.add-pack-cover-image')
        ...
        document
            .querySelector('.refresh-songs-one')
            .addEventListener('click', () => {
                this.refreshMySongs();
            });
        document.querySelector('.delete-pack').addEventListener('click', () => {
            if (this.jSon && this.currentPack) {
                this.jSon.packs = this.jSon.packs.filter(
                    p => p !== this.currentPack
                );
                this.bsaber.saveJson(this.jSon);
                this.refreshMySongs();
        ...
        document
            .querySelector('.add-pack-button')
            .addEventListener('click', () => {
                if (this.jSon) {
                    if (this.currentPack) {
                        this.currentPack.name = document.getElementById(
                            'packName'
                        ).value;
                        this.currentPack.coverImagePath =
                            this.packCoverPath || defaultImage;
                        delete this.currentPack;
                    } else {
                        this.jSon.packs.push({
                            id: this.jSon.packs.length,
                            name: document.getElementById('packName').value,
                            coverImagePath: this.packCoverPath || defaultImage,
                            levelIDs: [],
                        });
                    }
                    this.bsaber.saveJson(this.jSon);
        ...
        this.reset_patch_base.addEventListener('click', () => {
            ...
            if (copied) {
                ...
                this.showStatus(
                    'Error resetting, base does not exist!!',
                    true,
                    true
                );
            }
        ...
        this.patch_beatsaber.addEventListener('click', () => {
            ...
            if (
                ...
                this.patchModal.close();
                this.showStatus(
                    'Something went wrong, do you have songs selected?',
                    true,
                    true
                );
            }
        ...
        this.backupsModal = M.Modal.getInstance(
            document.querySelector('#modal5')
        );
        this.patchModal = M.Modal.getInstance(
            document.querySelector('#modal4')
        );
        this.packEditModal = M.Modal.getInstance(
        ...
        this.getMyBackups().then(async files => {
            ...
            if (files.length) {
                ...
                replaceTexts.forEach(replace => {
                    let child = document
                        .getElementById('replaceItem')
                        .content.cloneNode(true);
                    child.querySelector('.add-replace-key').innerText = replace;
                    child.querySelector('.add-replace-value').innerText =
                        jSon.replaceText[replace];
                    child
                        .querySelector('.remove-replace')
                        .addEventListener('click', () => {
                    ...
                    questSaberPatchContainer
                        .querySelector('#replaceTextValues')
                        .appendChild(child);
                });
                questSaberPatchContainer
                    .querySelector('.add-replace')
                    .addEventListener('click', () => {
                        jSon.replaceText[
                            questSaberPatchContainer.querySelector(
                                '#add-replace-key'
                            ).value
                        ] = questSaberPatchContainer.querySelector(
                            '#add-replace-value'
                ...
                let colorA = jSon.colors.colorA || {
                    r: 240.0 / 255.0,
                    g: 48.0 / 255.0,
                    b: 48.0 / 255.0,
                    a: 1,
                };
                let colorB = jSon.colors.colorB || {
                    r: 48.0 / 255.0,
                    g: 158.0 / 255.0,
                    b: 1,
                    a: 1,
                ...
                Pickr.create({
                    ...
                    default:
                        'rgba(' +
                        Math.round(colorA.r * 255) +
                        ',' +
                        Math.round(colorA.g * 255) +
                        ',' +
                        Math.round(colorA.b * 255) +
                        ',' +
                        colorA.a +
                        ')',
                ...
                Pickr.create({
                    ...
                    default:
                        'rgba(' +
                        Math.round(colorB.r * 255) +
                        ',' +
                        Math.round(colorB.g * 255) +
                        ',' +
                        Math.round(colorB.b * 255) +
                        ',' +
                        colorB.a +
                        ')',
                    defaultRepresentation: 'RGBA',
            ...
            files.forEach(f => {
                ...
                child
                    ...
                    .addEventListener('click', () => {
                        ...
                        this.setup
                            .installLocalApk(
                                path.join(appData, 'bsaber-backups', f)
                            )
                            .then(() => {
    ...
    async showInstalledPackage(child, app, appPackage) {
        ...
        if (~this.setup.devicePackages.indexOf(app.packageName)) {
            let installedVersion = await this.setup.getPackageInfo(
                app.packageName
            );
            let hasUpdate = false;
    ...
    async openAppScreen(app, appPackage) {
        ...
        try {
            appPackage.sort((a, b) =>
                semver.rcompare(a.versionName, b.versionName)
            );
        } catch (e) {}
                ...
                .addEventListener('click', () => {
                    ...
                    this.setup
                        ...
                        .then(() => {
                            this.toggleLoader(false);
                            this.showStatus('APK installed!!');
                            return this.showInstalledPackage(
                                this.container,
                                app,
                                appPackage
                            );
            ...
            this.setup.devicePackages
                ...
                .forEach((p, i) => {
                    ...
                    child
                        ...
                        .addEventListener('click', () => {
                            this.setup.getPackageLocation(p).then(location => {
                                if (location)
                                    this.setup.backupPackage(location, p);
                            });
                        });
                    this.container.appendChild(child);
                });
                ...
                if (err) {
                    ...
                    resolve(
                        (data || []).filter(
                            d =>
                                fs
                                    .lstatSync(path.join(backupDirectory, d))
                                    .isDirectory() &&
                                fs.existsSync(
                                    path.join(
                                        backupDirectory,
...
<span class="link" data-url="https://bsaber.com/members/elliotttate/">@elliotttate</span> for beat saber efforts.`;
    ...
    openDeviceSettings() {
        ...
        [].slice
            .call(document.querySelectorAll('.set-gpu-level'))
            .forEach(ele => {
                ele.addEventListener('click', () => {
                    let value = 2;
                    switch (ele.innerText.toLowerCase()) {
        ...
        [].slice.call(document.querySelectorAll('.set-ca')).forEach(ele => {
            ele.addEventListener('click', () => {
                ...
                this.spinner.setMessage(
                    'Setting Chromatic Aberration Setting...'
                );
                this.toggleLoader(true);
                this.setup.adb
                    .shell(
                        this.setup.deviceSerial,
                        '"setprop debug.oculus.forceChroma ' + value + '"'
                    )
                    .then(adb.util.readAll)
                    .then(res => {
                        this.showStatus(
        ...
        [].slice.call(document.querySelectorAll('.set-svb')).forEach(ele => {
            ele.addEventListener('click', () => {
                ...
                this.setup.adb
                    .shell(
                        this.setup.deviceSerial,
                        '"setprop debug.oculus.videoBitrate ' + value + '"'
                    )
                    .then(adb.util.readAll)
                    .then(res => {
        ...
        [].slice.call(document.querySelectorAll('.set-svr')).forEach(ele => {
            ele.addEventListener('click', () => {
                ...
                this.setup.adb
                    .shell(
                        this.setup.deviceSerial,
                        '"setprop debug.oculus.videoResolution ' + value + '"'
                    )
                    .then(adb.util.readAll)
                    .then(res => {
                        this.showStatus(
        ...
        document
            ...
            .addEventListener('change', function() {
                devicePaste.setAttribute(
                    'type',
                    this.checked ? 'password' : 'text'
                );
            });
        document
        ...
        document
            .querySelector('.open-apk-backup-folder')
            .addEventListener('click', () => {
                shell.openItem(path.join(appData, 'apk-backups'));
            });

        document
            .querySelector('.grant-pavlov-permission')
            .addEventListener('click', () => {
                this.setup.adb
                    .shell(
                        this.setup.deviceSerial,
                        'pm grant com.davevillz.pavlov android.permission.RECORD_AUDIO'
                    )
                    .then(adb.util.readAll)
                    .then(res => {
    ...
    openSetupScreen() {
        ...
        if (
            ...
            this.setup
                ...
                .then(() =>
                    this.setup.uninstallApk('com.openappstore.launcher')
                );
        }
        if (
            ~this.setup.devicePackages.indexOf('com.sidequest.wrapper2') &&
    ...
    searchFilter() {
        ...
        this.current_data.body.apps
            .filter(d => {
                ...
                let is_filter =
                    filter_value === d.category || filter_value === 'All';
                return (
            ...
            .forEach(d => {
                ...
                child.querySelector(
                    '.image'
                ).style.backgroundColor = randomColor({ seed: d.packageName });
                ...
                installApk.className =
                    'waves-effect waves-light btn install-apk';
                installApk.addEventListener('click', () => {
                    this.openAppScreen(
                        d,
                        this.current_data.body.packages[d.packageName]
                    );
                });
                if (~this.setup.devicePackages.indexOf(d.packageName)) {
                    child.querySelector('.is-installed').style.display =
                        'block';
                }
                this.container.appendChild(child);
            });
